...
function resolveSpawn(command, args, root) {
  // Shared hosts often upload node_modules/.bin/vite without +x (spawn EACCES).
  // Running the JS file through node never needs the execute bit.
  if (command === "vite") {
    return {
...
function main(argv) {
  ...
  const { cmd, args: childArgs } = resolveSpawn(command, args, root);
  // Windows spawn() does not resolve `.cmd` shims in node_modules/.bin without a shell.
