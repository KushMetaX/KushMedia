...
function normalizeSpecialMultipliers(raw = {}) {
  ...
  for (const combo of combosSource) {
    ...
    const conditions = Array.isArray(combo.conditions)
      ? combo.conditions.filter(c => c && typeof c === 'object' && c.trait && c.value)
      : [];
    if (conditions.length === 0) continue;
    const multiplier = Math.max(1.0, Number(combo.multiplier) || 1.0);
...
async function evaluateDog(dogNumber) {
  ...
  for (const combo of smCombos) {
    ...
    if (allMatch) {
      const cm = Math.max(1.0, Number(combo.multiplier) || 1.0);
      if (cm > comboMultiplier) { comboMultiplier = cm; matchedCombo = combo; }
    }
  }
  if (matchedCombo && estimatedDoge != null) {
  ...
  return {
    ...
    estimation: {
      ...
      method: [
        ...
        matchedCombo ? 'combo_multiplier' : null,
