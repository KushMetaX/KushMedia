...
If `DoginalDogsCatalog.json` is present, the evaluator prefers it for per-dog traits and derived trait counts instead of calling the live `wallet.getDogTraits` and `wallet.getTraitValues` endpoints for every lookup.

Recommended default location:

`server/data/DoginalDogsCatalog.json`

That path works without extra configuration in this repo and keeps the catalog out of the public `assets` directory.

Default lookup order:

1. `DD_CATALOG_PATH` when you want a custom location
2. `server/data/DoginalDogsCatalog.json`
3. `assets/DoginalDogsCatalog.json` for legacy/public fallback only

Set `DD_DISABLE_CATALOG=1` when you intentionally want live refreshes, such as rebuilding the catalog from market data.
